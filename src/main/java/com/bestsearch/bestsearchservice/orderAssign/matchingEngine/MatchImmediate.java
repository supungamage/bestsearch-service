...
public class MatchImmediate implements IMatchBehaviour {
  ...
  public void match() {
    ...
    timeFlyOrderAssignments.forEach(oa -> {
      ...
      orderIdVsDistance
          .put(oa.getOrderId(), oa.getOffsetPaginate()); //all radius shd be same for given orderId
    });
    ...
    orderIdVsDistance.forEach((id, distance) -> {
      ...
      if (Objects.nonNull(newAssignments)) {
        ...
        toBeSentOrders.addAll(newAssignments.stream().map(OrderAssignment::viewAsOrderAssignmentDTO)
